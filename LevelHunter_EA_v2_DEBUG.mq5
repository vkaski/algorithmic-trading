//+------------------------------------------------------------------+
//|                              LevelHunter_EA_DEBUG.mq5            |
//|                          Диагностическая версия                  |
//+------------------------------------------------------------------+
#property copyright "LevelHunter EA Debug"
...
input string InpLogFile        = "levellog_debug.csv";
...
int OnInit()
{
    ...
    UpdateLevels();
    Print("✅ UpdateLevels() завершён");

    Print("═══════════════════════════════════════════");
    Print("✅ LevelHunter EA v2 DEBUG запущен успешно");
    Print("═══════════════════════════════════════════");
    Print("Символ: ", _Symbol);
    Print("Структурный TF: ", EnumToString(InpStructureTF));
    Print("Entry TF: ", EnumToString(InpEntryTF));
    Print("Уровней найдено: ", g_level_count);
    Print("═══════════════════════════════════════════");
...
void OnDeinit(const int reason)
{
    ...
    Print("✅ LevelHunter v2 завершён. Причина: ", reason);
}

//+------------------------------------------------------------------+
//| OnTick (минимальная версия)                                     |
//+------------------------------------------------------------------+
void OnTick()
{
    ...
    {
        tick_count++;
        if(tick_count % 1000 == 0) // Каждые 1000 тиков
        {
            Print("[TICK] Обработано ", tick_count, " тиков на баре ", TimeToString(cur_bar));
        }
        return;
    }
    ...
    Print("[NEW BAR] ", TimeToString(cur_bar), " | Позиций: ", CountPositions(), 
          " | Уровней: ", g_level_count);

    static int bar_cnt = 0;
    bar_cnt++;
    if(bar_cnt >= g_structure_update_bars)
    {
        Print("   → Обновление уровней...");
        UpdateLevels();
        bar_cnt = 0;
    }
    ...
    if(!FindSignal(sig_dir, sig_level, sig_reason))
    {
        Print("   → Сигнал не найден");
        return;
    }

    Print("   → СИГНАЛ НАЙДЕН! ", sig_reason);
    ...
    {
        Print("❌ Ошибка входа: ", g_trade.ResultRetcode());
    }
}

//+------------------------------------------------------------------+
//| Вспомогательные функции (минимальные версии)                    |
//+------------------------------------------------------------------+
...
double CalculateLevelQuality(int level_idx)
{
    ...
    if(touches == 2) score += 15.0;
    if(touches == 3) score += 10.0;
    if(touches >= 4) score -= 10.0;
...
void ManageActiveTrades()
{
    ...
    for(int i = g_trade_count - 1; i >= 0; i--)
    {
        ...
        if(InpUseTrailingStop && profit_pts > (g_active_trades[i].initial_sl_pts * 0.5))
        {
            ...
            if(trailing_level > g_active_trades[i].initial_sl_pts)
            {
                        ...
                        const MqlTradeResult  &res)
{
    ...
    {
        if(g_active_trades[i].ticket == (ulong)HistoryDealGetInteger(deal, DEAL_POSITION_ID))
        {
