//+------------------------------------------------------------------+
//|                                   LevelHunter_EA_v2_ProFitKeeper |
//|                        Enhanced: Profit Protection & Risk Manager |
//|                                                                  |
//| УЛУЧШЕНИЯ:                                                       |
//|  1. Trailing Stop с защитой профита                              |
//|  2. Partial TP (30/70 or 40/60 split)                            |
//|  3. Volatility Filter для фильтрации слабых сигналов             |
//|  4. Trend Detection для подтверждения направления                |
//|  5. Trade Cooldown для предотвращения перепродаж                 |
//|  6. Max Daily Loss для стоп торговли при просаде                 |
//|  7. Enhanced Order Execution с проверкой слипиджа                |
//|  8. Level Quality Score для приоритизации уровней                |
//+------------------------------------------------------------------+
#property copyright "LevelHunter EA v2"
...
//--- ═══════════════════════════════════════════════════════════════
input group "=== Таймфрейм и структура ==="
...
//--- ═══════════════════════════════════════════════════════════════
input group "=== Зона уровня ==="
input double InpZonePoints     = 150.0;
input double InpTouchZone      = 120.0;

//--- ═══════════════════════════════════════════════════════════════
input group "=== Правила касаний ==="
input int    InpTypeA_MaxTouch = 2;
input int    InpTypeB_MaxTouch = 3;
input bool   InpTradeBreakout  = true;

//--- ═══════════════════════════════════════════════════════════════
input group "=== Подтверждение свечой ==="
input bool   InpUseCandleConfirm = true;
input double InpPinBarRatio    = 0.6;
input double InpEngulfRatio    = 1.1;

//--- ═══════════════════════════════════════════════════════════════
input group "=== 🆕 Volatility & Trend Filter ==="
input bool   InpUseVolatilityFilter = true;
input double InpMinVolatility  = 30.0;   // Минимальная ATR (пункты)
input double InpMaxVolatility  = 2000.0; // Максимальная ATR (не торговать в спайках)
input bool   InpUseTrendFilter = true;   // Только в направлении тренда?
input int    InpTrendMAPeriod  = 20;     // Период MA для детекции тренда

//--- ═══════════════════════════════════════════════════════════════
input group "=== 🆕 Profit Protection ==="
input bool   InpUsePartialTP   = true;   // Частичное TP
input double InpPartial1_Percent = 30.0; // % от позиции закрыть на 1-м TP
input double InpPartial1_RR    = 1.0;    // RR для 1-го частичного закрытия
input double InpPartial2_RR    = 2.5;    // RR для 2-го закрытия
input bool   InpUseTrailingStop = true;  // Трейлинг стоп
input double InpTrailingPercent = 50.0;  // % от профита для трейла (% от TP дистанции)
input int    InpTradeCooldownMin = 5;    // Минуты между сделками (избежать перепродажи)
...
input double InpRRRatio        = 1.8;    // Пониженный RR для лучшего сохранения профита
...
input double InpMaxDailyLossPercent = 5.0; // Стоп при просаде -5%
input int    InpMagic          = 202408;

//--- ═══════════════════════════════════════════════════════════════
input group "=== Hermes / Лог ==="
...
//+------------------------------------------------------------------+
//| Структура уровня с улучшениями                                  |
//+------------------------------------------------------------------+
struct PriceLevel
{
    double         price;
    ...
    datetime       last_touch;
    bool           used;
    double         quality_score;  // 🆕 Оценка качества уровня (0-100)
};

//+------------------------------------------------------------------+
//| Структура активной позиции для трейлинга                        |
//+------------------------------------------------------------------+
struct ActiveTrade
{
    ulong          ticket;
    ...
    double         sl;
    double         tp;
    double         partial_1_tp;  // 🆕 1-е частичное TP
    bool           partial_1_closed; // Закрыта ли часть?
    int            direction;
    datetime       open_time;
...
ActiveTrade   g_active_trades[];
int           g_trade_count = 0;

datetime      g_last_trade_time = 0;
datetime      g_last_structure_update = 0;
...
int OnInit()
{
    ...
    {
        Print("❌ Ошибка ATR"); return INIT_FAILED;
    }
    ...
    if(InpUseTrendFilter)
    {
        ...
        {
            Print("❌ Ошибка MA"); return INIT_FAILED;
        }
    }

    g_trade.SetExpertMagicNumber(InpMagic);
    g_trade.SetDeviationInPoints(50); // Повышенная толерантность к слипиджу
    ...
    Print("✅ LevelHunter EA v2 запущен | ", _Symbol,
          " | Profit Protection: ON | Trailing: ", InpUseTrailingStop ? "ON" : "OFF");
...
void OnDeinit(const int reason)
{
    ...
    Print("LevelHunter v2 завершён");
}

//+------------------------------------------------------------------+
//| Получить ATR в пунктах                                          |
//+------------------------------------------------------------------+
double GetATRPoints()
{
    double atr_buf[];
...
bool CheckVolatilityFilter()
{
    ...
    if(atr_pts < InpMinVolatility)
    {
        if(InpVerboseLog) Print("⚠️ Слишком низкая волатильность: ", DoubleToString(atr_pts, 1), " pts");
        return false;
    }
    if(atr_pts > InpMaxVolatility)
    {
        if(InpVerboseLog) Print("⚠️ Спайк волатильности: ", DoubleToString(atr_pts, 1), " pts");
        return false;
    }
...
//+------------------------------------------------------------------+
//| 🆕 Проверить тренд (простая MA)                                  |
//+------------------------------------------------------------------+
int CheckTrend()
{
    if(!InpUseTrendFilter) return 0; // 0 = нет фильтра
    ...
    // Если закрытие выше MA → растущий тренд (может быть BUY)
    // Если закрытие ниже MA → падающий тренд (может быть SELL)
    int trend = 0;
    if(close_buf[1] > ma_buf[1]) trend = 1;  // uptrend
    if(close_buf[1] < ma_buf[1]) trend = -1; // downtrend

    return trend;
}

//+------------------------------------------------------------------+
//| 🆕 Рассчитать качество уровня (0-100)                            |
//+------------------------------------------------------------------+
double CalculateLevelQuality(int level_idx)
{
    double score = 50.0; // базовая оценка

    // Тип A лучше, чем B
    if(g_levels[level_idx].ltype == LEVEL_TYPE_A)
        score += 20.0;
    else
        score += 10.0;

    // Больше касаний = менее надежный уровень (но в разумных пределах)
    int touches = g_levels[level_idx].touch_count;
    if(touches == 2) score += 15.0;       // идеально для A
    if(touches == 3) score += 10.0;       // нормально
    if(touches >= 4) score -= 10.0;       // ослабленный уровень

    // Недавний уровень лучше старого
    double age_days = (double)(TimeCurrent() - g_levels[level_idx].formed_time) / 86400.0;
    if(age_days < 1.0) score += 15.0;
    else if(age_days > 7.0) score -= 10.0;
...
bool CheckDailyLossLimit()
{
    ...
    {
        if(InpVerboseLog)
            Print("🛑 Достигнут дневной лимит убытков: ", DoubleToString(g_daily_loss, 2));
        return false;
    }
...
//+------------------------------------------------------------------+
//| 🆕 Проверить cooldown между сделками                             |
//+------------------------------------------------------------------+
...
void UpdateLevels()
{
    ...
    PriceLevel new_levels[];
    int new_count = 0;
    ArrayResize(new_levels, 500);

    for(int i = sw; i < bars; i++)
    {
        // Swing High
        bool is_sh = true;
        for(int j = 1; j <= sw; j++)
        ...
        if(is_sh)
        {
            ...
            for(int k = 0; k < new_count; k++)
                if(MathAbs(new_levels[k].price - highs[i]) < zone) { dup = true; break; }

            if(!dup)
            {
                bool impulse = IsImpulseSwing(i, LEVEL_HIGH);
                new_levels[new_count].price       = highs[i];
                new_levels[new_count].ltype       = impulse ? LEVEL_TYPE_A : LEVEL_TYPE_B;
                new_levels[new_count].direction   = LEVEL_HIGH;
                new_levels[new_count].touch_count = 1;
                new_levels[new_count].broken      = false;
                new_levels[new_count].retested    = false;
                new_levels[new_count].formed_time = times[i];
                new_levels[new_count].last_touch  = times[i];
                new_levels[new_count].used        = false;
                new_count++;
            }
        }
        ...
        if(is_sl)
        {
            bool dup = false;
            for(int k = 0; k < new_count; k++)
                if(MathAbs(new_levels[k].price - lows[i]) < zone) { dup = true; break; }

            if(!dup)
            {
                bool impulse = IsImpulseSwing(i, LEVEL_LOW);
                new_levels[new_count].price       = lows[i];
                new_levels[new_count].ltype       = impulse ? LEVEL_TYPE_A : LEVEL_TYPE_B;
                new_levels[new_count].direction   = LEVEL_LOW;
                new_levels[new_count].touch_count = 1;
                new_levels[new_count].broken      = false;
                new_levels[new_count].retested    = false;
                new_levels[new_count].formed_time = times[i];
    ...
    // Перенести данные и рассчитать качество
    for(int n = 0; n < new_count; n++)
    {
        for(int o = 0; o < g_level_count; o++)
        {
            if(MathAbs(g_levels[o].price - new_levels[n].price) < zone)
            {
                new_levels[n].touch_count = g_levels[o].touch_count;
                new_levels[n].broken      = g_levels[o].broken;
                new_levels[n].retested    = g_levels[o].retested;
                new_levels[n].used        = g_levels[o].used;
                new_levels[n].last_touch  = g_levels[o].last_touch;
                break;
            }
        }
        new_levels[n].quality_score = CalculateLevelQuality(n);
    }

    ArrayResize(g_levels, new_count);
    for(int i = 0; i < new_count; i++)
        g_levels[i] = new_levels[i];
    g_level_count = new_count;

    if(InpVerboseLog)
        Print("📐 Уровней: ", g_level_count, " | Качество: ", 
              (g_level_count > 0 ? DoubleToString(g_levels[0].quality_score, 1) : "N/A"));
}
...
//+------------------------------------------------------------------+
//| FindSignal с дополнительной фильтрацией                         |
//+------------------------------------------------------------------+
bool FindSignal(int &signal_dir, double &signal_level, string &reason)
{
    ...
    // Отсортировать уровни по качеству
    int best_idx = -1;
    ...
    reason = StringFormat("Q%.0f|%s|lvl=%.2f",
             g_levels[best_idx].quality_score,
             g_levels[best_idx].ltype == LEVEL_TYPE_A ? "A" : "B",
             g_levels[best_idx].price);

    return true;
}

//+------------------------------------------------------------------+
//| 🆕 Управление активными позициями (трейлинг, partial TP)       |
//+------------------------------------------------------------------+
void ManageActiveTrades()
{
    ...
    for(int i = g_trade_count - 1; i >= 0; i--)
    {
        ...
        // 🆕 Частичное закрытие на первом TP
        if(InpUsePartialTP && !g_active_trades[i].partial_1_closed)
        {
            double tp1_dist = g_active_trades[i].initial_sl_pts * InpPartial1_RR;
            if(profit_pts >= tp1_dist)
            {
                double partial_lot = g_pos.Volume() * (InpPartial1_Percent / 100.0);
                if(partial_lot >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
                {
                    if(g_active_trades[i].direction > 0)
                        g_trade.Sell(partial_lot, _Symbol, 0, 0, 0, "PARTIAL-TP1");
                    else
                        g_trade.Buy(partial_lot, _Symbol, 0, 0, 0, "PARTIAL-TP1");
                    
                    g_active_trades[i].partial_1_closed = true;
                    if(InpVerboseLog)
                        Print("💰 Частичное закрытие на TP1: ", DoubleToString(partial_lot, 2), " lot");
                }
            }
        }

        // 🆕 Трейлинг стоп
        if(InpUseTrailingStop && profit_pts > (g_active_trades[i].initial_sl_pts * 0.5))
        {
            double tp_dist = g_active_trades[i].initial_sl_pts * InpRRRatio;
            double trailing_level = (g_active_trades[i].max_profit - tp_dist * (InpTrailingPercent / 100.0));
            
            if(trailing_level > g_active_trades[i].initial_sl_pts) // только выше исходного SL
            {
                double new_sl = (g_active_trades[i].direction > 0) ?
                               NormalizeDouble(current_price - trailing_level * pt, (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS)) :
                               NormalizeDouble(current_price + trailing_level * pt, (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS));

                if((g_active_trades[i].direction > 0 && new_sl > g_active_trades[i].sl) ||
                   (g_active_trades[i].direction < 0 && new_sl < g_active_trades[i].sl))
                {
                    g_trade.PositionModify(g_active_trades[i].ticket, new_sl, g_active_trades[i].tp);
                    g_active_trades[i].sl = new_sl;
                }
            }
        }
    }
...
void OnTick()
{
    ...
    if(result)
    {
        ...
        // 🆕 Добавить позицию в массив для управления
        if(g_pos.SelectByTicket(ticket))
        {
            ArrayResize(g_active_trades, g_trade_count + 1);
            ...
            Print(StringFormat("✅ %s | Lot=%.2f | SL=%.0f TP=%.0f pts | %s",
                  sig_dir > 0 ? "BUY" : "SELL", lot, sl_pts, tp_pts, sig_reason));
        }
    }
    else
        Print("❌ Ошибка: ", g_trade.ResultRetcode());
}
...
//+------------------------------------------------------------------+
//| OnTradeTransaction (с записью в лог и учёт потерь)              |
//+------------------------------------------------------------------+
                        ...
                        const MqlTradeResult  &res)
{
    ...
    g_daily_loss += profit; // 🆕 Учитывать дневные потери
    ...
    WriteLog(pos_id, profit, sl_pts, tp_pts, dir, HistoryDealGetString(deal, DEAL_COMMENT));

    string status = (profit > 0) ? "💚 WIN" : "🔴 LOSS";
    Print(StringFormat("%s | P/L=%+.2f | Daily: %+.2f", status, profit, g_daily_loss));

    // Удалить закрытую позицию из массива активных
    for(int i = 0; i < g_trade_count; i++)
    {
        if(g_active_trades[i].ticket == (ulong)pos_id)
        {
...
//| WriteLog                                                         |
//+------------------------------------------------------------------+
void WriteLog(long ticket, double profit, double sl_pts, double tp_pts,
              int dir, string reason)
{
    int fh = FileOpen(InpLogFile, FILE_READ | FILE_WRITE | FILE_CSV | FILE_ANSI | FILE_SHARE_READ);
    if(fh == INVALID_HANDLE)
        fh = FileOpen(InpLogFile, FILE_WRITE | FILE_CSV | FILE_ANSI);
    ...
    FileWrite(fh,
        ...
        (string)ticket, _Symbol,
        (dir > 0 ? "BUY" : "SELL"),
        DoubleToString(bid, _Digits),
        DoubleToString(sl_pts, 0),
        DoubleToString(tp_pts, 0),
